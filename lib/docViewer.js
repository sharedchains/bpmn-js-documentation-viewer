...
export default function DocViewer(eventBus, overlays) {
  ...
  function createDocButton(element) {
    ...
    let documentation = element.businessObject.$attrs['documentation:extendedDocumentation'];
    ...
    document.body.appendChild(getEmptyModal());
    ...
    $overlay.on('click', (event) => {
      if (typeof ($.fn.modal) === 'function') {
        ...
        if (documentation && documentation.length > 0) {
          $(".modal-body").empty();
          $(".modal-body").append(documentation);
          $("#myModal").modal();
        }
        
      } else {
        //console.log("NO BOOTSTRAP HERE");
