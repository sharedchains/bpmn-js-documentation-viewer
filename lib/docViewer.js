...
export default function DocViewer(eventBus, overlays, elementRegistry, bpmnjs) {
  ...
  function createDocButton(element) {
    ...
    let documentation = element.businessObject.get('documentation');
    if (documentation.length > 0) {
      $overlay.addClass('full');
    }

    if (badgeOverlay) {
      overlays.get({ type: 'doc-badge' }).forEach(badge => {
    ...
    $overlay.on('click', (event) => {
      ...
      if (documentation.length > 0) {
        ...
        modal.find('.doc-modal-content').append(documentation[0].text);
...
DocViewer.$inject = ['eventBus', 'overlays', 'elementRegistry', 'bpmnjs'];